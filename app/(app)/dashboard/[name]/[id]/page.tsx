// app/(app)/dashboard/[name]/[id]/page.tsx
"use client";
...
export default function DashboardPage() {
  ...
  >([]);
  const [rpt_dynamictables, setrptDynamictables] = useState<
    ReportItem["dynamic_tables"]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (siteData) {
        ...
        const inputsWithIds: DynamicInputItem[] = siteData.dynamic_inputs.map(
          (item, index) => ({
            ...item,
            id: (item as any).id || Date.now() + index,
          }),
        );
        const rptinputsWithIds: DynamicInputItem[] =
          siteData?.rpt_inputs?.map((item, index) => ({
            ...item,
            id: (item as any).id || Date.now() + index,
          })) || [];
          ...
          {sitedata && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-2 p-2 bg-background text-foreground">
                <div className="p-4 shadow-md border shadow-gray-200 rounded-md bg-background text-foreground items-center flex justify-between gap-1">
                  {sitedata && (
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    <SaveIndicator
                      saving={statusSave.saving}
                      saved={statusSave.saved}
                      error={statusSave.error}
                    />
                    <Threewaytoggle
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        <p>NB:</p>
                        <p>
                          • To disable a shift, set both the shift start and
                          shift end to <strong>23:59</strong>.
                        </p>
                        <p>
                          • To enable a 24-hour shift, ensure a normal day shift
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>
                          Select main scales for reporting
                        </CardDescription>
                      </CardHeader>
                      <CardContent>
                ...
                <TabsContent value="rpt" className="space-y-4 mt-4">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Rand Per Ton Configuration
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <Rpt
                        rptInputs={rptdynamicInputs}
                        rpt_dynamictables={rpt_dynamictables}
                      />
                    </CardContent>
                  </Card>
                </TabsContent>
