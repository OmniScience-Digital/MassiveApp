...
const months = Array.from({ length: 12 }, (_, i) => dayjs().month(i).format("MMMM")); // Month names

const DatePicker = ({ date, setDate,title }: { date: string;title: string; setDate: (value: string) => void }) => {
  ...
  return (
    ...
    <Label className="text-sm font-normal text-foreground">{title}</Label>
      <div className="flex items-center gap-2">
        {/* Year Selector */}
        <Select onValueChange={handleYearChange} defaultValue={selectedDate.year().toString()}>
        ...
        {/* Month Selector */}
        <Select onValueChange={handleMonthChange} defaultValue={months[selectedDate.month()]}>
      ...
      <Popover>
        ...
        <PopoverContent align="start" className="p-0">
          <Calendar mode="single" selected={selectedDate.toDate()} onSelect={handleDayChange} />
        </PopoverContent>
      </Popover>
    </div>
  );
