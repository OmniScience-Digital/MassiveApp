...
function listsites() {
  ...
  client.models.Sites.observeQuery().subscribe({
    next: (data) => {
      const sites = data.items.map((report) => {
        const parsedSite = typeof report.site === "string" 
          ? JSON.parse(report.site) 
          : {};
        parsedSite.id = report.id;
        return parsedSite;
      }).filter(site => site?.siteConstants?.reporttype === "stockpile").sort(); // Filter where reporttype is telegram;
  ...
  const handleDelete=async(id:string)=>{
   ...
   const redirectToDashboard = (name: string,id:string) => {
    let path = name.replace(/\s+/g, '');
     ...
     router.push(`/stockpileDashboard/${path}/${id}`);
  };
...

    {
      ...
      cell: ({ row }: { row: any }) => (
        <Button
          ...
          onClick={() => redirectToDashboard('stockpile',row.original.id)} // Log the row ID
        >
          <EditIcon />
        </Button>
      ),
        ...
        <Checkbox
          ...
          onCheckedChange={async (checked) => {
            ...
            try {
              ...
              console.error(`Failed to update audit status for site ID: ${siteId}`, err);
            }
          }}
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 p-1 h-f mt-20">
        <div className="flex justify-end mb-2">
          ...
          <InputModal isOpen={isModalOpen} reporttype={'stockpile'} onClose={handleCloseModal} onSubmit={handleSubmit} />
        </div>
