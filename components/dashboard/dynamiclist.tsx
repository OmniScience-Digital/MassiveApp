...
export default function Home() {
  ...
  const addDashboard = async () => {
    ...
    setOpen(false);
    setLoading(true);
    const { errors, data: newDashboard } =
      await client.models.Dashboards.create({
        items: name,
      });
    setName("");
  ...
  const deleteDashboard = async (index: number) => {
    ...
    if (foundItem) {
      ...
      const { errors } = await client.models.Dashboards.delete({
        id: foundItem.id,
      });
  ...
  return (
    <div className="flex flex-col bg-background text-foreground p-1">
      <div className="flex-1">
        {!loading && (
          <h1 className="text-2xl font-bold mb-4">Manage Dashboards</h1>
        )}
