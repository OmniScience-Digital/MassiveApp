...
import { createDynamicList, deleteDynamicList } from "@/service/dynamiInpuList.Service";
...
interface InputListProps {
  ...
  setDynamicInputs: React.Dispatch<React.SetStateAction<ReportItem["dynamic_inputs"]>>;
  setInputListCount: React.Dispatch<React.SetStateAction<number>>;

}


const InputList = ({ initialHeaderName = "Dynamic Input List", initialInputs = [], setDynamicInputs, setInputListCount, inputListCount }: InputListProps) => {
  ...
  const addInput = (type: InputType) => {
    setInputs([...inputs, { type, value: "", label: `Field ${inputs.length + 1}`, isEditing: false }]);
  ...
  const handleDeleteComponent = async () => {
    try {
      ...
      const deleteComponent = await deleteDynamicList(id as string, structuredData);
    ...
    <div className="px-6 py-2 border h-full w-full rounded-lg shadow-sm bg-background space-y-4">
      ...
      <div className="flex items-center justify-between">
        {isEditingHeader ? (
          ...
          <h2 className="text-xm font-medium uppercase tracking-wide text-foreground" onClick={toggleEditHeader}>
            {headerName}
          </h2>
        )}
        ...
        <Dialog open={showInputDialog} onOpenChange={setShowInputDialog}>
          ...
          <DialogContent className="max-w-sm">
            <DialogHeader>
              ...
              <DialogDescription>Please select available inputs.</DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <Button onClick={() => addInput("datetime-local")}>DateTime</Button>
            </div>
          </DialogContent>
        </Dialog>
          ...
          <div key={index} className="flex flex-wrap items-center ">
            <div className="flex-1 flex flex-wrap items-center ">
              {input.isEditing ? (
                ...
                <span className="w-full  text-sm font-medium">{input.label}</span>
              )}
            ...
            <Button variant="ghost" size="icon" onClick={() => toggleEdit(index)}>
              {input.isEditing ? <Save className="h-4 w-4" /> : <Edit className="h-4 w-4" />}
            </Button>
            <Button variant="ghost" size="icon" onClick={() => deleteInput(index)}>
      ...
      <div className="flex justify-end">
            ...
            {loadingDelete ? ( <Button className="mr-2" disabled>
                 ...
                 <Button onClick={handleDeleteComponent} variant="destructive" className="mx-3">
