        ...
        <div className="min-h-screen flex flex-col">
            ...
            <main className="flex-1 p-1 mt-20">
                <div className="container mx-auto p-6 max-w-6xl">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-2xl font-bold">CSV Upload & Simulator Control</CardTitle>
                            <CardDescription>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="flex gap-2 mb-4 flex-wrap">
                                <Button onClick={() => fileInputRef.current?.click()} disabled={isLoading}>
                                    {isLoading ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Upload className="w-4 h-4 mr-1" />} 
                                    {isLoading ? 'Loading...' : 'Browse CSV'}
                                </Button>
                                <Button onClick={sendToBackend} disabled={isSending || !csvData} className="bg-green-600 hover:bg-green-700">
                                ...
                                <Button onClick={stopSimulation} variant="destructive" disabled={!status?.isRunning}>
                                    Stop Simulator
                                </Button>
                            </div>
                            ...
                            {connectionError && (
                                <div className="mt-4 p-4 border rounded bg-amber-50 border-amber-200">
                                    ...
                                    <p className="text-sm mt-2">Please ensure the backend server is running and try again.</p>
                                </div>
                            )}
                            ...
                            {status ? (
                                <div className="mt-4 p-4 border rounded bg-gray-50">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            ...
                                            <p className={status.isRunning ? "text-green-600" : "text-red-600"}>
                                ...
                                <div className="mt-4 p-4 border rounded bg-gray-50 text-center text-gray-500">
                                    No status data available. Click "Check Status" to fetch current status.
                                </div>
                            )}
                        </CardContent>
                    </Card>
