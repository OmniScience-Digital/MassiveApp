...
async function toggleSiteField(siteId: string, field: string, value: boolean) {
  ...
  await client.models.Sites.update({
    id: siteId,
    site: JSON.stringify({ ...parsed, [field]: value }),
  });
}
...
function getUniqueStopTimes(sites: ReportItem[], excludeTimes = ["23:59"]) {
  ...
  sites.forEach(({ siteTimes }) => {
    if (siteTimes.dayStop && !excludeTimes.includes(siteTimes.dayStop))
      d.add(siteTimes.dayStop);
    if (siteTimes.nightStop && !excludeTimes.includes(siteTimes.nightStop))
      n.add(siteTimes.nightStop);
    if (
      siteTimes.extraShiftStop &&
      !excludeTimes.includes(siteTimes.extraShiftStop)
    )
      e.add(siteTimes.extraShiftStop);
  });
...
const Automatedreporting = () => {
  ...
  useEffect(() => {
    ...
    const toMap = (f: keyof ReportItem) =>
      submittedsites.reduce(
        (acc, s) => {
          acc[s.id] = Boolean(s[f]);
          return acc;
        },
  ...
  const handleSubmit = async (data: ReportItem) => {
    setLoading(true);
    try {
      const { errors } = await client.models.Sites.create({
  ...
  const handleDeleteConfirmation = async () => {
    try {
      setOpendelete(false);
      setLoading(true);
      if (idtodelete) await client.models.Sites.delete({ id: idtodelete });
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const filteredData = submittedsites
    .filter(
      (s) =>
        statusFilter === "all" ||
        (s.siteStatus || "off").toLowerCase() === statusFilter,
    )
    .map((site) => ({
      id: site.id || "",
  ...
  const makeCheckboxCol = (
    ...
    const allChecked =
      filteredData.length > 0 && filteredData.every((r) => state[r.id]);
    return {
      accessorKey: key,
      header: () => (
        <div className="flex flex-col items-center gap-1">
          ...
          <Checkbox
            ...
            onCheckedChange={async (checked) => {
              ...
              await Promise.all(
                filteredData.map((r) => toggleSiteField(r.id, key, val)),
              );
            }}
          />
        </div>
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return (
          <div className="flex items-center justify-center gap-1">
            <Checkbox
              checked={state[siteId] || false}
              onCheckedChange={async (checked) => {
                const isChecked = Boolean(checked);
                setState((prev) => ({ ...prev, [siteId]: isChecked }));
                setSavingField(fk);
                try {
                  await toggleSiteField(siteId, key, isChecked);
                } catch {
                  setState((prev) => ({ ...prev, [siteId]: !isChecked }));
                } finally {
                  setSavingField(null);
  ...
  const columns: ColumnDef<object, any>[] = [
    ...
    },
    {
      ...
      header: "Status",
      cell: ({ row }: { row: any }) => (
        <StatusBadge status={row.original.siteStatus} />
      ),
    },
    {
      accessorKey: "actions",
      ...
      cell: ({ row }: { row: any }) => (
        <div className="flex items-center gap-1">
          <Button
            ...
            title="Edit site"
            onClick={() =>
              router.push(`/dashboard/telegram/${row.original.id}`)
            }
          >
    ...
    makeCheckboxCol(
      "progressive",
      "Progressive",
      checkedItemsProgressive,
      setCheckedItemsProgressive,
    ),
    makeCheckboxCol(
      "hourly",
      "Hourly",
  ...
  const prodCount = submittedsites.filter(
    (s) => (s.siteStatus || "").toLowerCase() === "prod",
  ).length;
  const testCount = submittedsites.filter(
    (s) => (s.siteStatus || "").toLowerCase() === "test",
  ).length;
  const offCount = submittedsites.filter(
    (s) => (s.siteStatus || "off").toLowerCase() === "off",
  ).length;
      ...
      {loading ? (
        ...
        <>
          ...
          <DataTable
            ...
            columns={columns}
            pageSize={10}
            storageKey="telegramSitesTablePagination"
          />
        </>
      )}
      <ConfirmDialog
