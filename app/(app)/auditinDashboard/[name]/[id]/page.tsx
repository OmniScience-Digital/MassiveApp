...
const AuditingDashboard = () => {
  ...
  const getPurpleTableValues = async (
    startTime: string,
    endTime: string,
  ): Promise<{
    ...
    try {
      ...
      const { data: purplevalues, errors } =
        await client.models.PurplefigureTable.listPurplefigureTableBySiteIdAndRowdate(
          {
            siteId: id,
            rowdate: {
              between: [startDate, endDate],
      ...
      purplevalues.forEach((purplevalue) => {
        ...
        try {
          ...
          if (parsedValues && typeof parsedValues === "object") {
            Object.entries(parsedValues).forEach(([hour, value]) => {
              purpleValuesMap[iccid][date][hour] =
        ...
        // Store day total
        dayTotalsMap[iccid][date] =
          parseFloat(purplevalue.dayTotal || "0") || 0;
      });
  ...
  const fetchInputValues = useCallback(
    async (startTime: string, endTime: string) => {
      try {
        ...
        const loadedPurpleValues: Record<
          string,
          Record<string, Record<string, number>>
        > = purpleData.purpleValues;
        const loadedDayTotals: Record<
          string,
          Record<string, number>
        > = purpleData.dayTotals;
        ...
        if (matchedPurpleKey) {
          ...
          Object.entries(loadedDayTotals[matchedPurpleKey] || {}).forEach(
            ([date, total]) => {
  ...
  useEffect(() => {
    ...
    if (matchedPurpleKey) {
      ...
      const currentDayTotals: Record<string, number> = {};
      Object.entries(allDayTotals[matchedPurpleKey] || {}).forEach(
        ([date, total]) => {
          currentDayTotals[date] = total;
        },
      );
  ...
  const fetchData = async (startTime: string, endTime: string) => {
    try {
      ...
      const allIccids = Array.from(
        new Set(
          filteredSiteData.flatMap((day) =>
            day.scales.map((scale) => scale.iccid),
          ),
        ),
      );
  ...
  useEffect(() => {
    if (
      allIccids.length > 0 &&
      currentIccidIndex >= 0 &&
      currentIccidIndex < allIccids.length
    ) {
