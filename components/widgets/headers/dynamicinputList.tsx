...
import { createSiteHeaders, deleteSiteHeader } from '@/service/siteHeaders.Service';
  ...
  const handleSave = async () => {
    try {
      ...
      if (isValid) {
        ...
        const createheaders = await createSiteHeaders(id, { headername: inputs[index].headername });
      ...
      {inputs.map((input, index) => (
        <div key={index} className="flex items-center space-x-2">
          <Input
            ...
            className="w-64"
          />
          <Button onClick={() => handleRemoveInput(index)} variant="destructive">
            Remove
          </Button>
