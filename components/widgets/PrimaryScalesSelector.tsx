...
export const PrimaryScalesSelector = ({ scales, primaryScales, onSave }: PrimaryScalesSelectorProps) => {
    ...
    const handleRemoveCustomScale = async (scaleName: string, e: React.MouseEvent) => {
        e.stopPropagation();

        // Update UI immediately
        if (persistedCustomScales.includes(scaleName)) {
            setPersistedCustomScales(prev => prev.filter(name => name !== scaleName));
        } else {
    ...
    const handleSave = async () => {
        try {
            ...
            setSuccessful(false);
            setMessage(error instanceof Error ? error.message : 'Failed to update scales');
            setShow(true);
    ...
    const allDisplayScales = [
        ...scales,
        ...persistedCustomScales.map(scalename => ({ scalename, iccid: `custom-${scalename}` })),
        ...customScales.map(scalename => ({ scalename, iccid: `temp-${scalename}` }))
    ];
                        ...
                        {allDisplayScales.map(scale => (
                            <div key={scale.iccid} className="flex items-center space-x-2">
                                <Checkbox
                                    ...
                                    checked={selectedScales.includes(scale.scalename) && !removedScales.includes(scale.scalename)}
                                ...
                                >
                                    ...
                                    {(persistedCustomScales.includes(scale.scalename) || 
                                     customScales.includes(scale.scalename)) && (
                                        <button 
                                            onClick={(e) => handleRemoveCustomScale(scale.scalename, e)}
