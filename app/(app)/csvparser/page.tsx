...
const CSVParser = () => {
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (parsedData.rows.length > MAX_ROWS) {
        setMessage(
          `CSV exceeds maximum row limit of ${MAX_ROWS}. Found ${parsedData.rows.length} rows.`,
        );
        setShow(true);
        setSuccessful(false);
        setIsLoading(false);
      ...
      if (parsedData.headers.length > MAX_COLUMNS) {
        setMessage(
          `CSV exceeds maximum column limit of ${MAX_COLUMNS}. Found ${parsedData.headers.length} columns.`,
        );
        setShow(true);
        setSuccessful(false);
        setIsLoading(false);
  ...
  const sendToBackend = async () => {
    ...
    const globalStartRowIndex = csvData.rows.findIndex(
      (row: string[]) => row[1] === "1" || row[1] === "1.00",
    );
    ...
    try {
      ...
      if (errors.length === 0) {
        ...
        setMessage(
          `✅ Success! Uploaded ${totalProcessed} rows in ${totalChunks} chunks ,starting publish shortly`,
        );
        setSuccessful(true);
      } else {
        setMessage(
      ...
      setMessage(
        "Connection Error: Unable to connect to the CSV parser backend.",
      );
      setShow(true);
      setSuccessful(false);
    } finally {
  ...
  return (
    <>
      <div className="container mx-auto p-6 max-w-6xl">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl font-bold">CSV Parser</CardTitle>
            <CardDescription>
              Upload CSV (max {MAX_ROWS} rows, max {MAX_COLUMNS - 2} columns),
              then submit by sending.
            </CardDescription>
          </CardHeader>
