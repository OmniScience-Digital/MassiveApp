...
export default function Breadcrumbs() {
  ...
  if (pathSegments.length === 3 && (pathSegments[0] === 'auditing' || pathSegments[0] === 'progressivereporting')) {
        ...
        {pathSegments.map((segment, index) => {
          ...
          return (
            <li key={fullPath} className="flex items-center">
              ...
              {isLast ? (
                <span className="text-primary font-medium">{decodeURIComponent(segment)}</span>
              ) : (
