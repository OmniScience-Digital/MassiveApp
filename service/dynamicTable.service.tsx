...
export const createrptDynamicTable = async (
  id: string,
  table: ReportItem["dynamic_tables"],
) => {
  try {
    ...
    // Step 3: Ensure `dynamic input list` exists as an array
    if (!Array.isArray(parsedSite.rpt_tables)) {
      parsedSite.rpt_tables = [];
    }
    ...
    // Step 4: Check if table exists and update or add new
    table.forEach((newTable: ReportItem["dynamic_tables"][number]) => {
      const existingIndex = parsedSite.rpt_tables.findIndex(
        (t: ReportItem["dynamic_tables"][number]) => t.id === newTable.id,
      );

      if (existingIndex >= 0) {
        // Update existing table
        parsedSite.rpt_tables[existingIndex] = {
          ...parsedSite.rpt_tables[existingIndex],
          ...newTable,
        };
        ...
        // Add new table
        parsedSite.rpt_tables.push(newTable);
        message = "Table created successfully";
      }
    });
    ...
    return { message: message, table: parsedSite.rpt_tables };
  } catch (error) {
    console.log(error);
    throw error;
  }
...
export const deleterptDynamicTable = async (id: string, deleteId: number) => {
  try {
    // Step 1: Fetch the existing site data
    const { data: site, errors } = await client.models.Sites.get({ id });
    ...
    if (!Array.isArray(parsedSite.rpt_tables)) {
      console.error("dynamic_tables is not an array");
      parsedSite.rpt_tables = []; // Initialize if missing
    }

    // Step 4: Find the index of the table to delete
    const tableIndex = parsedSite.rpt_tables.findIndex(
      (table: ReportItem["dynamic_tables"][number]) => table.id === deleteId,
    );
    ...
    // Step 5: Remove the table from the array
    const deletedTable = parsedSite.rpt_tables.splice(tableIndex, 1)[0];

    // Step 6: Update the site with the modified rpt_tables
    const updateResult = await client.models.Sites.update({
      id: site.id,
      site: JSON.stringify(parsedSite),
    });
