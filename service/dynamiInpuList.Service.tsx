...
export const createRptDynamicList = async (
  id: string,
  inputobj: {
    inputListName: string;
    inputs: InputData[];
  ...
  try {
    ...
    // Step 3: Ensure `dynamic input list` exists as an array
    if (!Array.isArray(parsedSite.rpt_inputs)) {
      parsedSite.rpt_inputs = [];
    }
    ...
    // Step 5: Find if the inputListName already exists
    const existsIndex = parsedSite.rpt_inputs.findIndex(
      (I: any) =>
        I.inputListName.trim().toLowerCase() === normalizedInputListName,
    );
    ...
    if (existsIndex !== -1) {
      // Do NOT allow renaming, only update inputs
      parsedSite.rpt_inputs[existsIndex].inputs = inputobj.inputs;
      ...
      parsedSite.rpt_inputs.push(inputobj);
    }
...
export const deleteRptDynamicList = async (
  id: string,
  inputobj: {
    inputListName: string;
    inputs: InputData[];
  ...
  try {
    ...
    // If the input list exists, update it; otherwise, push a new one
    if (existsIndex !== -1) {
      existsIndex > -1 && existsIndex < parsedSite.rpt_inputs.length
        ? parsedSite.rpt_inputs.splice(existsIndex, 1)
        : null;
    } else {
      return -1;
    }
